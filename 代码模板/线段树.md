...
假设我们有一个数组 `arr[]`，其长度为 `n`。线段树是基于这个数组构建的一个完全二叉树，每个节点代表数组的一个区间。叶子节点存储的是数组的单个元素，而内部节点存储的是它子节点的区间和。
...
线段树的大小大约为 `4 * n`，所以我们通常用长度为 `4 * n` 的数组来存储线段树。

### 2. 线段树的操作

线段树通常有两个主要操作：

1. **区间查询**：查询数组的某个区间的和、最大值、最小值等。
2. **单点更新或区间更新**：更新数组中的某个元素或某个区间的值。

#### (1) 构建线段树

构建线段树的过程是递归的，自底向上构建。假设根节点负责存储整个数组区间 `[0, n-1]`，我们递归地将区间一分为二，左子树存储区间 `[0, mid]`，右子树存储区间 `[mid+1, n-1]`，如此递归直到每个叶子节点。
...
区间查询用于查询数组的某个区间 `[L, R]` 的和。这个查询过程也是递归的，关键是如何处理以下三种情况：

1. **完全包含**：查询区间完全包含当前节点的区间，直接返回该节点的值。
2. **无交集**：查询区间和当前节点的区间无交集，返回零或无效值。
3. **部分重叠**：查询区间和当前节点的区间有部分重叠，递归查询左右子树，合并查询结果。
...
class SegmentTree:
    ...
    def _query(self, node, start, end, L, R):
        # 如果查询区间[L, R]与当前节点的区间[start, end]没有交集，返回0
        if start > R or end < L:
            return 0
        # 如果查询区间[L, R]完全包含当前节点的区间[start, end]，返回当前节点的值
...
更新操作是将数组中的某个元素更新为新的值，并且需要更新相关的线段树节点。这也是一个递归过程，通过查找并更新对应的叶子节点，然后更新它的父节点直到根节点。
...
1. **构建线段树**：每个节点的构建涉及到合并两个子节点的操作，时间复杂度为 O(n)，因为树的大小为 `4 * n`。
2. **区间查询**：区间查询的时间复杂度为 O(log n)，因为每次查询会递归遍历线段树的高度，而线段树的高度为 `log n`。
3. **单点更新**：更新操作的时间复杂度也是 O(log n)，因为更新操作也是递归进行，树的高度为 `log n`。
...
# 查询区间[1, 3]的和
print(seg_tree.query(1, 3))  # 输出：15 (3 + 5 + 7)

# 更新索引 2 处的值为 6
seg_tree.update(2, 6)

# 查询更新后的区间[1, 3]的和
print(seg_tree.query(1, 3))  # 输出：16 (3 + 6 + 7)
```

### 5. 线段树的变种

- **Lazy Propagation（懒惰标记）**：用于处理区间更新的优化。如果需要更新一个区间而不是单个点，可以使用懒惰标记（lazy propagation），将更新延迟到真正需要时再执行，从而避免频繁的递归更新操作。

线段树是一种非常强大且灵活的数据结构，能够高效解决各种区间查询和修改问题。
...
线段树（Segment Tree）是一种用于高效处理区间查询和更新的数据结构，特别适合处理区间求和、区间最值、区间最大公约数等操作。它支持**动态更新**，可以在**O(log n)**的时间复杂度内进行区间查询和单点更新。
